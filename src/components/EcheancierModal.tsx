...
export function EcheancierModal({ projectId, onClose, formatCurrency, formatDate }: EcheancierModalProps) {
  ...
  useEffect(() => {
    ...
    const fetchEcheances = async () => {
      setLoading(true);
      try {
        // Récupérer toutes les souscriptions du projet - SANS jointures imbriquées
        const { data: subscriptionsData, error: subsError } = await supabase
          .from('souscriptions')
          .select('id, id_souscription, investisseur_id, tranche_id')
        ...
        // Récupérer les investisseurs séparément
        const investisseurIds = [...new Set(subscriptionsData.map(s => s.investisseur_id))];
        ...
        investisseursData?.forEach(inv => {
          investisseursMap.set(inv.id, inv);
        });

        // Récupérer les tranches séparément
        const trancheIds = [...new Set(subscriptionsData.map(s => s.tranche_id))];
        const { data: tranchesData } = await supabase
          .from('tranches')
          .select('id, tranche_name')
          .in('id', trancheIds);

        const tranchesMap = new Map();
        tranchesData?.forEach(tr => {
        ...
        const enrichedEcheances = (echeancesData || []).map(ech => {
          ...
          const tranche = sub ? tranchesMap.get(sub.tranche_id) : null;
          ...
          return {
            ...
            souscription: {
              ...
              tranche: tranche || { tranche_name: 'Inconnue' }
            }
          };
        });
