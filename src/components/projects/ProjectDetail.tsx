...
export function ProjectDetail({ organization: _organization }: ProjectDetailProps) {
  ...
  const { user: _user, isSuperAdmin, userRole, loading: authLoading } = useAuth();
  ...
  const fetchProjectData = async () => {
    ...
    try {
      ...
      const subscriptionsWithCoupons = subscriptionsData.map(sub => {
        const prochainCoupon = prochainsCouponsData.find(pc => pc.souscription_id === sub.id);
        return {
          ...sub,
      ...
      setAlertState({
        isOpen: true,
        title: 'Erreur',
        message: `Erreur lors du chargement des données: ${error instanceof Error ? error.message : String(error)}`,
        type: 'error',
      });
    } finally {
      setLoading(false);
  ...
  const performProjectUpdate = async (hasFinancialChanges: boolean) => {
    try {
      ...
      setAlertState({
        isOpen: true,
        title: 'Erreur',
        message: `Erreur lors de la mise à jour : ${err instanceof Error ? err.message : String(err)}`,
        type: 'error',
      });
    }
  };
      ...
      onConfirm: async () => {
        try {
          ...
          setAlertState({
            isOpen: true,
            title: 'Erreur',
            message: `Erreur lors de la suppression : ${err instanceof Error ? err.message : String(err)}`,
            type: 'error',
          });
        }
      },
