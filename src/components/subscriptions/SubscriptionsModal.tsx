...
interface SubscriptionsModalProps {
  ...
  onDelete: (sub: Record<string, unknown>) => void;
...
export function SubscriptionsModal({
  ...
  const filteredSubs = useMemo(() => {
    ...
    result.sort((a, b) => {
      ...
      if (sortBy === 'date') {
        compareValue =
          new Date(a.date_souscription).getTime() - new Date(b.date_souscription).getTime();
      } else if (sortBy === 'montant') {
        compareValue = a.montant_investi - b.montant_investi;
      } else if (sortBy === 'investisseur') {
        compareValue = a.investisseur.nom_raison_sociale.localeCompare(
          b.investisseur.nom_raison_sociale
        );
      }
