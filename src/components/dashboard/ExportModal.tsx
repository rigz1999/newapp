...
interface ExportModalProps {
  ...
  dashboardData: {
    ...
    upcomingCoupons: Record<string, unknown>[];
    alerts: { type: string; message: string; severity?: string }[];
    monthlyData: { month: string; amount: number; cumulative?: number }[];
  };
}
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      if (options.includeCoupons && coupons.length > 0) {
        ...
        const couponsByProject = coupons.reduce(
          (
            acc: Record<
              string,
              {
            ...
            const tranche = coupon.tranche as { projet?: { projet?: string } } | undefined;
            const projectName = fixTextForPDF(tranche?.projet?.projet || 'N/A');
            if (!acc[projectName]) {
              acc[projectName] = {
                projectName,
                coupons: [],
            ...
            acc[projectName].totalAmount += parseFloat(String(coupon.coupon_brut));
        ...
        projectGroups.forEach((group, _index: number) => {
          if (yPos > 250) {
            doc.addPage();
            yPos = 25;
          }
          ...
          const nextCoupon = group.coupons.sort(
            (a: Record<string, unknown>, b: Record<string, unknown>) =>
              new Date(String(a.prochaine_date_coupon)).getTime() -
              new Date(String(b.prochaine_date_coupon)).getTime()
          )[0];
          const daysUntilNext = Math.ceil(
            (new Date(String(nextCoupon.prochaine_date_coupon)).getTime() - new Date().getTime()) /
              (1000 * 60 * 60 * 24)
          );
          ...
          doc.text(
            `• Prochain: ${formatDate(String(nextCoupon.prochaine_date_coupon))} (dans ${daysUntilNext} jour${daysUntilNext > 1 ? 's' : ''})`,
            SPACING.MARGIN + 3,
            yPos + 10
          );
          ...
          if (group.count > 1) {
            const subsequentDates = group.coupons
              .slice(1, 3)
              .map((c: Record<string, unknown>) => formatDate(String(c.prochaine_date_coupon)))
              .join(', ');
            if (subsequentDates) {
              doc.setFontSize(8);
